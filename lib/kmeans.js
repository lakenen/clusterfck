...
KMeans.prototype.classify = function(point, distance) {
   ...
   for (var i = 0; i < this.centroids.length; i++) {
      var dist = distance(point.values, this.centroids[i].values);
      if (dist < min) {
         min = dist;
         index = i;
      }
...
KMeans.prototype.cluster = function(points, k, distance, snapshotPeriod, snapshotCb) {
   ...
   while (movement) {
      ...
      for (var j = 0; j < k; j++) {
         ...
         var newCentroid = { values: new Array(centroid.values.length) };

         for (var g = 0; g < centroid.values.length; g++) {
            var sum = 0;
            for (var i = 0; i < assigned.length; i++) {
               sum += assigned[i].values[g];
            }
            newCentroid.values[g] = sum / assigned.length;

            if (newCentroid.values[g] != centroid.values[g]) {
               movement = true;
            }
         }
